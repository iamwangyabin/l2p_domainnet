...
def load_and_custom_init_checkpoint(*, config: ml_collections.ConfigDict,
    ...
    if model_type.startswith("ViT"):
        ...
        loaded_param_dict["head"]["kernel"] = init_state.optimizer.target["head"]["kernel"]
        loaded_param_dict["head"]["bias"] = init_state.optimizer.target["head"]["bias"]
        optimizer = init_state.optimizer.replace(
...
def transfer_weights(config, param_dict, task_id, kernel_only=True):
    ...
    param_dict["head"]["kernel"] = (
        param_dict["head"]["kernel"].at[:, task_id * num_classes_per_task:(task_id + 1) *  num_classes_per_task]
    ...
    if not kernel_only:
        ...
        param_dict["head"]["bias"] = (
            param_dict["head"]["bias"].at[task_id * num_classes_per_task:(task_id + 1) *  num_classes_per_task]
